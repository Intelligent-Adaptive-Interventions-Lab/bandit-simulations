...
class TSPostDiffPolicySimple(Policy):
    def __init__(self, policy_configs: Dict, bandit: Bandit) -> None:
        super().__init__(policy_configs, bandit)
    ...
    def run(self, new_learner: str, new_learner_df: Dict={}) -> Dict:
        new_learner_df[self.get_learner_column_name()] = new_learner
        new_learner_df[self.get_burn_in_column_name()] = int(self.params.is_burn_in())
    ...
    def get_reward(self, new_learner_df: List[Dict]) -> Dict:
        true_arm_probs = dict(self.params.parameters["true_arm_probs"])
        reward = self.bandit.reward

        # Update reward for the new learner dataframe.
        for learner_df in new_learner_df:
            arm_name = learner_df[self.get_arm_column_name()]
            true_reward = np.random.binomial(reward.max_value - reward.min_value, true_arm_probs[arm_name]) + reward.min_value
            learner_df[reward.name] = reward.get_reward(true_reward)
        
        return new_learner_df

    def update_params(self, assignment_np: np.ndarray) -> np.ndarray:
        # Record update batch indicator.
        assignment_np[:, -1] = self.update_count

        reward = self.bandit.reward

        # Only consider two arm case in postdiff
        # TODO: generalize success and failure update for more than 2 arm cases
        reward_np = assignment_np[:, self.columns.index(self.get_reward_column_name())]
        action_np = assignment_np[:, self.columns.index(self.bandit.get_actions()[0])]

        for index, row in self.bandit.arm_data.arms.iterrows():
            is_pos = row[self.bandit.get_actions()[0]]
            if is_pos:
                print("positive: {}".format(np.sum(np.multiply(reward_np,action_np))))
                sum_rewards = np.sum(np.multiply(reward_np, action_np))
                count_rewards = np.sum(action_np)
            else:
                sum_rewards = np.sum(reward_np) - np.sum(np.multiply(reward_np, action_np))
                count_rewards = len(action_np) - np.sum(action_np)
            ...
            assignment_np[:, self.columns.index("{} Success".format(arm_name).replace(" ", "_").lower())] = success_update
            assignment_np[:, self.columns.index("{} Failure".format(arm_name).replace(" ", "_").lower())] = failure_update

            # curr_count_rewards = assignment_np[:, self.columns.index("{} Count".format(arm_name).replace(" ", "_").lower())]
            # assignment_np[:, self.columns.index("{} Count".format(arm_name).replace(" ", "_").lower())] = curr_count_rewards + count_rewards
        
        # Update the indicator.
        self.update_count += 1

        return assignment_np