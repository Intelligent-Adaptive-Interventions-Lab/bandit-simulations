...
from policies.tscontextual.ts_contextual import thompson_sampling_contextual, thompson_sampling_contextual_simple
...
class TSContextualPolicySimple(Policy):
    ...
    def __init__(self, policy_configs: Dict, bandit: Bandit) -> None:
        ...
        self.columns = [ 
            self.get_burn_in_column_name(), 
            self.get_reward_column_name()
        ] + self.bandit.get_actions() + self.bandit.get_contextual_variables() + [self.get_udpate_batch_column_name()]
        
    def get_parameter_column_names(self) -> List[str]:
        return ["coef_cov", "coef_mean", "variance_a", "variance_b", "precesion_draw", "coef_draw"]
    
    def get_contexts(self) -> Dict:
        return dict([(var, self.bandit.contexts_dict[var].get_rvs()) for var in self.bandit.contexts_dict])

    def run(self, new_learner_df: Dict={}) -> Dict:
        # Generate contexts
        if not new_learner_df:
            new_learner_df = self.get_contexts()
        context_dict = {key: new_learner_df[key] for key in self.bandit.get_contextual_variables()}

        new_learner_df[self.get_burn_in_column_name()] = int(self.params.is_burn_in())

        # Get best action and datapoints (e.g. assigned arm, generated contexts) for the new learner.
        best_action = thompson_sampling_contextual_simple(self.params, context_dict)
    ...
    def get_reward(self, new_learner_df: List[Dict]) -> Dict:
        ...
        for learner_df in new_learner_df:
            raw_reward = calculate_outcome(learner_df, np.array(true_coef_mean), include_intercept, true_estimate)
            if reward.value_type == "BIN":
                true_reward = np.random.binomial(1, raw_reward, 1)[0]
            else:
                # Get scale of error.
                ...
                error = np.random.normal(0, err_scale, 1)[0]
                true_reward = raw_reward + error
            learner_df[reward_name] = reward.get_reward(true_reward)

        return new_learner_df

    def update_params(self, assignment_df: np.ndarray) -> np.ndarray:
        # Record update batch indicator.
        assignment_df[:, -1] = self.update_count

        # Update parameters.
        self.params.update_params_simple(assignment_df, self.columns)
